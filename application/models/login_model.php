...
class Login_model extends CI_Model{
    ...
    function loginMe($email, $password){
        ...
        $this->db->from('smc_users as BaseTbl');
        $this->db->join('smc_roles as Roles','Roles.roleId = BaseTbl.roleId');
    ...
    function resetPasswordUser($data){
        $result = $this->db->insert('smc_reset_password', $data);
    ...
    function checkActivationDetails($email, $activation_id){
        $this->db->select('id');
        $this->db->from('smc_reset_password');
        $this->db->where('email', $email);
    ...
    function createPasswordUser($email, $password){
        $this->db->where('email', $email);
        $this->db->where('isDeleted', 0);
        $this->db->update('smc_users', array('password'=>getHashedPassword($password)));
        $this->db->delete('smc_reset_password', array('email'=>$email));
    }
}
